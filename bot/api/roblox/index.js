// Roblox API helpers (legacy; may be split further)
...
import {
  ...
  getRobloxAccessToken,
} from "./auth.js";

var priv_listener = false
const ROBLOX_OAUTH_SETTINGS = buildRobloxAuthSettings();

function hasLegacyRoblosecurity() {
  return Boolean(String(COOKIE || "").trim());
}

async function getRobloxBearerToken() {
  try {
    const tokenInfo = await getRobloxAccessToken({
      settings: ROBLOX_OAUTH_SETTINGS,
      tokenStorePath: ROBLOX_OAUTH_SETTINGS.tokenStorePath,
    });
    return String(tokenInfo?.accessToken || "").trim();
  } catch {
    return "";
  }
}

async function robloxFetchOAuthFirst(
  url,
  requestOptions = {},
  { csrf = false, allowLegacyFallback = true } = {}
) {
  ...
  const canUseLegacy = allowLegacyFallback && hasLegacyRoblosecurity();
  const bearerToken = await getRobloxBearerToken();
  ...
  if (bearerToken) {
    ...
    if (
      oauthResponse.ok ||
      !canUseLegacy ||
      ![401, 403].includes(Number(oauthResponse.status || 0))
    ) {
      return { response: oauthResponse, authMode: "oauth" };
    }
  }
  ...
  if (canUseLegacy) {
    ...
    if (csrf && method !== "GET" && method !== "HEAD") {
      const csrfToken = await getXcsrf();
      if (csrfToken) legacyHeaders["X-CSRF-TOKEN"] = csrfToken;
    }
    ...
    return { response: legacyResponse, authMode: "legacy_cookie" };
  }
  ...
  return { response, authMode: "none" };
}

export const getXcsrf = async (cookie = COOKIE) => {
  ...
  // Roblox returns x-csrf-token on purpose when you don't include one
  return String(r.headers.get("x-csrf-token") || "").trim();
};
...
const PRESENCE_GENERIC_LOCATIONS = new Set([
  "",
  "roblox",
  "website",
  "roblox studio",
...
const PRESENCE_GAME_NAME_CACHE_TTL_MS = 5 * 60 * 1000;
const presenceGameNameCache = new Map();
const LEGACY_PRESENCE_CACHE_TTL_MS = 4_000;
const legacyPresencePayloadCache = new Map();

export function cleanPresenceDisplayName(value = "") {
  let name = String(value || "")
    .replace(/[\r\n]+/g, " ")
    .trim();
  if (!name) return "";

  // Remove bracketed tags like [VC], (BETA), {NEW}, etc.
  name = name.replace(/\s*[\[\(\{][^\]\)\}]{1,80}[\]\)\}]\s*/g, " ");
...
function readPresenceGameNameCache(key) {
  const entry = presenceGameNameCache.get(String(key || ""));
  if (!entry) return "";
  if (Number(entry.expiresAt || 0) <= Date.now()) {
    presenceGameNameCache.delete(String(key || ""));
    return "";
  }
  return cleanPresenceDisplayName(entry.name);
}

function writePresenceGameNameCache(key, name) {
  const cacheKey = String(key || "").trim();
  const value = cleanPresenceDisplayName(name);
  if (!cacheKey || !value) return;
  presenceGameNameCache.set(cacheKey, {
    name: value,
    expiresAt: Date.now() + PRESENCE_GAME_NAME_CACHE_TTL_MS,
  });
}

function normalizePresenceTypeFallback(type) {
  if (type === 3) return "Roblox Studio";
  if (type === 2) return "Roblox";
  return "Website";
}

export function isGenericPresenceLocation(value = "") {
  return PRESENCE_GENERIC_LOCATIONS.has(String(value || "").trim().toLowerCase());
}

async function fetchUniverseGameName(universeId) {
  const id = Number(universeId || 0);
  ...
  try {
    ...
    return String(d?.data?.[0]?.name || "").trim();
  } catch {
    return "";
  }
}

async function fetchPlaceGameName(placeId) {
  const id = Number(placeId || 0);
  if (!Number.isInteger(id) || id <= 0) return "";

  try {
    const r = await fetch(
      `https://games.roblox.com/v1/games/multiget-place-details?placeIds=${encodeURIComponent(
        id
      )}`
    );
    if (!r.ok) return "";
    ...
    return String(rows?.[0]?.name || rows?.[0]?.placeName || "").trim();
  } catch {
    return "";
  }
}
...
export async function resolvePresenceLocation(presence = {}) {
  ...
  if (location && !isGenericPresenceLocation(location)) {
    return cleanPresenceDisplayName(location) || location;
  }

  if (type !== 2) {
    return cleanPresenceDisplayName(location) || normalizePresenceTypeFallback(type);
  }
  ...
  if (cacheKey) {
    const cached = readPresenceGameNameCache(cacheKey);
  ...
  resolvedName = cleanPresenceDisplayName(resolvedName);

  if (resolvedName && cacheKey) {
    writePresenceGameNameCache(cacheKey, resolvedName);
  }

  return resolvedName || cleanPresenceDisplayName(location) || "Roblox";
}

function normalizePresencePayload(p = null) {
  const type = typeof p?.userPresenceType === "number" ? p.userPresenceType : 0;
  const rawLocation = String(p?.lastLocation ?? "").trim();
  const normalizedLocation = rawLocation
    ? cleanPresenceDisplayName(rawLocation) || rawLocation
    : type === 2
      ? "Roblox"
      : type === 3
        ? "Roblox Studio"
...
function shouldEnrichPresenceWithLegacy(presence = null) {
  const type = Number(presence?.userPresenceType ?? 0);
...
async function fetchPresenceLegacyPayload(userId) {
  if (!hasLegacyRoblosecurity()) return null;
  const cacheKey = String(Number(userId || 0));
  const now = Date.now();
  const cached = legacyPresencePayloadCache.get(cacheKey);
  ...
  try {
    ...
    if (!r.ok) return null;
    const d = await r.json().catch(() => null);
    const payload = d?.userPresences?.[0] || null;
    legacyPresencePayloadCache.set(cacheKey, {
      payload,
      expiresAt: now + LEGACY_PRESENCE_CACHE_TTL_MS,
    });
    return payload;
  } catch {
    return null;
...
async function resolvePresencePayloadWithLegacy(userId, payload) {
  const normalized = normalizePresencePayload(payload);
  if (!shouldEnrichPresenceWithLegacy(normalized)) {
    return normalized;
  }

  const legacyPayload = await fetchPresenceLegacyPayload(userId);
  if (!legacyPayload) return normalized;

  return normalizePresencePayload(legacyPayload);
}
...
export const getPresence = async (userId) => {
  // Safe fallback so your callers never explode
  const fallback = normalizePresencePayload(null);

  try {
    const { response: r } = await robloxFetchOAuthFirst(
    ...
    if (!r.ok) {
      // optional logging:
      // console.log("getPresence non-ok:", r.status);
      return fallback;
    }

    const d = await r.json();

    // ✅ Guard: userPresences missing/empty
    const p = d?.userPresences?.[0];
    if (!p) return fallback;

    return await resolvePresencePayloadWithLegacy(userId, p);
  } catch (e) {
    // optional logging:
...
function safePresence(d) {
  const p = d?.userPresences?.[0];
  const normalized = normalizePresencePayload(p);

  return {
    ok: !!p, // handy for debugging/logic
    userPresenceType: normalized.userPresenceType,
    lastLocation: normalized.lastLocation,
    placeId: normalized.placeId,
    universeId: normalized.universeId,
    raw: d ?? null, // optional: keep raw response for logs
  };
}
...
export function getPresenceSync (userId,cb) {
    (async () => {
      ...
      try {
        if (!userId) return cb(fallback);
        const presence = await getPresence(userId);
        cb(presence || fallback);
      } catch {
        cb(fallback);
      }
    })();
...
export const monitorGetPresence = async (userId) => {
  if (!userId) return safePresence(null);
  ...
  try {
    ...
    console.log("monitorGetPresence fetch failed:", err);
    return safePresence(null);
  }
};

  
export function monitorGetPresenceSync(userId, cb) {
  if (!userId) return cb(safePresence(null));
  ...
  (async () => {
    try {
      ...
      console.log("monitorGetPresenceSync fetch failed:", err);
      cb(safePresence(null));
    }
  })();
}
...
export const sendFriendRequest = async (userId) => {
  const { response: r, authMode } = await robloxFetchOAuthFirst(
  ...
  // Known cases Roblox sends as errors
  if (msg.includes("already a friend") || msg.includes("already exists")) return "already";
  if (r.status === 429 || msg.includes("too many requests")) return "rate_limited";

  if (!r.ok) {
    console.log("sendFriendRequest status:", r.status, authMode);
    console.log("sendFriendRequest json:", JSON.stringify(json, null, 2));
    return "error";
  }
...
export const removeFriend = async (userId) => {
  ...
  const msg = String(json?.errors?.[0]?.message || json?.message || "").toLowerCase();

  if (msg.includes("not friends")) return "not_friends";
  if (r.status === 429 || msg.includes("too many requests")) return "rate_limited";

  if (!r.ok) {
    console.log("removeFriend status:", r.status, authMode);
    if (text) console.log("removeFriend body:", text.slice(0, 300));
    return "error";
  }
...
export const getCurrentUserFriends = async(userId = 3562575828)=>{
    const targetUserId = Number(userId) > 0 ? Number(userId) : 3511204536;
    const { response: r } = await robloxFetchOAuthFirst(
      `https://friends.roblox.com/v1/users/${targetUserId}/friends?userSort=Alphabetical`,
      {
        headers: {
          "Accept": "application/json, text/plain, */*",
          "Accept-Language": "en-GB,en;q=0.5",
